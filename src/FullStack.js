...
class FullStack extends React.Component{
      ...
      componentDidMount(){
        ...
        .then(response=>{
            ...
            const fullStackDevelopers= response.data.filter(ele=>ele.jobTitle == "FULL Stack Developer")
    ...
    handleRejected = (id) => {
      ...
      return (
        <div>
          ...
          <h3> List of candidates applied for full stack developer job</h3>
          <table border='1'>
            <thead>
                <tr>
                    <th> Name</th>
            ...
            <tbody>
                {
                   this.state.fullStackDevelopers.map((ele,i) =>{
                       return(
                            <tr key={i}>
                                <td> {ele.name}</td>
                                <td> {ele.skills}</td>
